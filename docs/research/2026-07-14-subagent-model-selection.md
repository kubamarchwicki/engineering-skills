...
Codex documents reasoning settings from low through high/xhigh, with some models or surfaces also supporting Max or Ultra. Higher effort takes more time and tokens; Ultra is more than a per-agent deliberation setting because it can proactively delegate to subagents. ([OpenAI, Models](https://learn.chatgpt.com/docs/models), [OpenAI, Subagents](https://learn.chatgpt.com/docs/agent-configuration/subagents)) The durable `model_reasoning_effort` configuration reference currently lists `minimal`, `low`, `medium`, `high`, and `xhigh`, so project agent TOML should stay within those documented values; treat Max and Ultra as session/UI modes unless the active interface explicitly accepts them. ([OpenAI, Configuration Reference](https://learn.chatgpt.com/docs/config-file/config-reference))
